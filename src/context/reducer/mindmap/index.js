...
export const defaultValue_mindmap =
  JSON.parse(localStorage.getItem("mindmap")) || defaultMindmap;
  ...
  switch (action.type) {
    ...
    case actionTypes.ADD_SIBLING: {
      ...
      if (action.data.parent_id) {
        ...
        const insert_index =
          node_found.children.findIndex(
            (node) => node.id === action.data.node_id
          ) + 1;
    ...
    case actionTypes.MOVE_NODE: {
      const new_mindmap = deepCopy(mindmap);
      const parent = findNode(new_mindmap, action.data.parent_id);
      if (parent === undefined) return mindmap;
      const node_index = parent.children.findIndex(
        (node) => node.id === action.data.node_id
      );
      ...
      if (action.data.is_sibling) {
        const target_index =
          parent.children.findIndex(
            (node) => node.id === action.data.target_id
          ) + 1 || parent.children.length + 1;
    ...
    case actionTypes.DELETE_NODE: {
      ...
      if (action.data.parent_id) {
        const node_found = findNode(new_mindmap, action.data.parent_id);
        const delete_index = node_found.children.findIndex(
          (node) => node.id === action.data.node_id
        );
