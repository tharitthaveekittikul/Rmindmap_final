  ...
  const handleKeyEventWithNode = (event) => {
    switch (event.key.toUpperCase()) {
      ...
      case " ":
        ...
        toggleChildren(
          nodeStatus.cur_select,
          !nodeStatus.cur_node_info.showChildren
        ); // Boolean('false') === true
        break;
      case "ARROWLEFT":
      ...
      case "ARROWUP": {
        event.preventDefault();
        const cur_index = nodeStatus.cur_node_info.parent.children.findIndex(
          (child) => child.id === nodeStatus.cur_node_info.id
        );
        if (cur_index > 0) {
          selectNode(
            nodeStatus.cur_node_info.parent.children[cur_index - 1].id
          );
        }
        break;
      }
  ...
  return (event) => {
    ...
    if (nodeStatus.cur_select !== "") {
      try {
        ...
        alert(
          "There is a problem with the current node information, please re-select the node"
        );
      }
    }
  };
