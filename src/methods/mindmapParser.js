...
const buildNodeFromText = (data_array, format, cur_layer, cur_text = "") => {
  ...
  if (cur_layer === -1) {
    ...
    cur_text = root_data.text || "Unknown data";
  }
  const cur_node = {
    id:
      cur_layer === 0
        ? refer.ROOT_NODE_ID
        : md5("" + Date.now() + Math.random() + cur_text),
    text: cur_text,
    showChildren: true,
    children: [],
  };
  while (data_array.length > 0) {
    ...
    if (layer) {
      // 排除掉无法匹配的情况
      cur_node.children.push(
        buildNodeFromText(data_array, format, layer, text)
      );
    }
  }
  return cur_node;
...
const copyNodeData = (format, target_node, source_node, is_root_node) => {
  switch (format) {
    case "KM":
      ...
      target_node.children = source_node.children.map((child) =>
